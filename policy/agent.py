...
class Agent(PolicyBase):
    def __init__(self, env, n_hidden, log, tb_writer, name, args):
        super(Agent, self).__init__(
            env=env, n_hidden=n_hidden, log=log, tb_writer=tb_writer, name=name, args=args)
    ...
    def set_dim(self):
        ...
        self.log[self.args.log_name].info("[{}] number of hidden neurons: {}".format(
            self.name, self.n_hidden))
