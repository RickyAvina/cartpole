...
def test(agent, env, log, tb_writer, args):
    ...
    while True:
        ...
        # Take action
        new_obs, env_reward, done, _ = env.step(agent_action)
...
def train(agent, env, log, tb_writer, num_samples, args):
    global total_eps

    while True:
        # Collect one trajectory
        collect_one_trajectory(agent=agent, env=env, log=log,
                               tb_writer=tb_writer, args=args)
        tb_writer.add_scalar("debug/memory", len(agent.memory), total_eps)

        # Update policy every `num_samples` cycles
...
def collect_one_trajectory(agent, env, log, tb_writer, args):
    ...
    while True:
        ...
        if done:
            ...
            tb_writer.add_scalars("reward", {"ep_reward": ep_reward},
                                  total_eps)
            tb_writer.add_scalars("reward", {"running_reward": running_reward},
                                  total_eps)
